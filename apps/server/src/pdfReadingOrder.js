 ...
 */
export function orderPdfText(boxes, asBlocks = false) {
  if (!boxes.length) return asBlocks ? [] : '';
  ...
  const render = list => asBlocks ? list.flatMap(line => line.parts) : list.map(line => {
  ...
  flush();
  return asBlocks ? sections.flat() : sections.join('\n\n');
}
